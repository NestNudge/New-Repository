...
$chatLeadsFile = $dataDir . '/chat-leads.json';
...
if (!file_exists($chatLeadsFile)) {
    file_put_contents($chatLeadsFile, json_encode([], JSON_PRETTY_PRINT));
}
...
function normalizeLeadType(string $projectType, string $source): string
{
    ...
    if (str_contains($value, 'bathroom')) return 'contractor';
    if (str_contains($value, 'kitchen')) return 'contractor';
    if (str_contains($value, 'landscaping')) return 'contractor';
    if (str_contains($value, 'entry-door')) return 'contractor';
    if (str_contains($value, 'impact-windows')) return 'contractor';
...
function requestLabelFromLeadType(string $leadType): string
{
    $map = [
        ...
        'contractor' => 'Speak with an Expert regarding a Home Improvement Project',
...
if (stripos($source, 'AI') !== false) {
    $chatLeads = loadLeadFile($chatLeadsFile);
    $chatLeads[] = $leadRecord;
    saveLeadFile($chatLeadsFile, $chatLeads);
}
...
logActivity('lead_dispatched', 'lead-handler.php', [
    'lead_id' => $leadRecord['id'],
    'matched_count' => $dispatchResult['matched_count'] ?? 0,
    'sent_count' => $dispatchResult['sent_count'] ?? 0,
